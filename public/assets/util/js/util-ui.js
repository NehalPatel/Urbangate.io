...
$.fn.restaurant_action = function(is_saved , success , error)
{
	...
	{
		if(g['userId'] && $dev_save.data('show-popover'))
...
$.fn.user_action = function(is_saved , success , error)
{
	...
	$dev_save.click(function()
	{
		...
		if(typeof user_followers != "undefined")
		{
			if(active)
			{
				user_followers.push(userId);
			}
			else
			{
				var index = user_followers.indexOf(userId);
				if (index > -1)
				{
		...
		$dev_save.toggleClass('active');
		post_data('user/save_user' , {userId : userId , active : active} , function(response)
		{
			if(!response.status)
			{
				$dev_save.toggleClass('active');

...
class CreateWingCommitteeMembersTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('com_wing_committee_members', function (Blueprint $table) {
            $table->id('auto_id');
            ...
            $table->string('wing_committee_id', 50);
            $table->foreign('wing_committee_id')->references('wing_committee_id')->on('com_wing_committees');

            $table->string('member_id', 50);
            $table->foreign('member_id')->references('member_id')->on('com_members');

            $table->string('committee_member_type_id');
    ...
    {
        Schema::dropIfExists('com_wing_committee_members');
    }
}
